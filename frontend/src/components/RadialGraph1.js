...
export function RadialGraph1() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${process.env.SERVER_URL}/api/cve/exploited/stats`);
        if (!response.ok) {
  ...
  if (error) {
    return (
      <Card className="flex h-full w-full flex-col rounded-lg bg-slate-950/75 backdrop-blur-sm">
